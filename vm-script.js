...
let API_URL = ''; // Will be set by user input
let allNotes = [];
...
$(document).ready(function() {
    ...
    $('#connectBtn').on('click', connectToBackend);
    $('#reconnectBtn').on('click', function() {
        $('#connectionModal').removeClass('hidden').addClass('flex');
    });
    
    // Load connection from localStorage if available
    const savedHost = localStorage.getItem('backendHost');
    const savedPort = localStorage.getItem('backendPort');
    if (savedHost && savedPort) {
        $('#backendHost').val(savedHost);
        $('#backendPort').val(savedPort);
...
function connectToBackend() {
    const host = $('#backendHost').val().trim();
    const port = $('#backendPort').val().trim();
    ...
    testConnection()
        .then(() => {
            // Hide modal
            $('#connectionModal').removeClass('flex').addClass('hidden');
            // Load notes
            getListNotes();
            // Update status
        ...
        .catch(err => {
            ...
            if (connectionAttempts < 3) {
                ...
                showToast('Could not connect to backend. Please check the IP address and port.', 'error');
                ...
                $('#notesList').html(`
                    <div class="col-span-full p-8 bg-red-50 rounded-lg border border-red-200 text-center">
                        ...
                        <p class="text-red-600 mb-4">Make sure your backend server is running and accessible from this VM.</p>
